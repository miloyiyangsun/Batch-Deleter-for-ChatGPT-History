...
(async () => {
    ...
    // --- 配置区 ---
    // 你可以在这里方便地调整所有等待时间，单位为毫秒 (ms)
    const TIME_TO_WAIT_FOR_UI_TRANSITION = 50; // 用于等待菜单、弹窗等UI元素出现的延时
    const TIME_TO_WAIT_FOR_PAGE_REFRESH = 100; // 用于等待删除操作后页面刷新的延时
    // -------------

    // 从 session storage 中获取用户设定的删除数量
    const storageResult = await chrome.storage.session.get('deleteCount');
    const numberOfDeletions = storageResult.deleteCount || 3; // 如果未设置，默认为 3
    ...
    /**
     * 最终版的自动化删除函数。
     * V4: 先记录前 N 个聊天标题作为锚点，然后根据锚点逐个定位并执行完整的删除流程。
     */
    async function deleteByTextAnchors() {
        console.log(`🚀 V4: 开始执行基于文本锚点的删除流程，目标数量: ${numberOfDeletions}...`);
        ...
        // 提取前 N 个有效标题作为我们的目标锚点 (N 由用户输入决定)
        for (let i = 0; i < Math.min(allLinks.length, numberOfDeletions); i++) {
        ...
        for (const title of targetTitles) {
            ...
            await delay(TIME_TO_WAIT_FOR_PAGE_REFRESH); // 等待更长时间，让删除后的刷新完成
        }
