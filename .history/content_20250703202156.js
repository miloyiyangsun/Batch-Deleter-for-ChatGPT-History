...
(async () => {
    ...
    console.log("🚀 启动自动化删除脚本 (V3 - 单元化处理)...");
    ...
    async function runAutomation() {
        ...
        for (let i = 0; i < itemsToDelete.length; i++) {
            ...
            // 复用我们成熟的操作链
            console.log(`[步骤 1/4] 悬停以显示"选项"按钮...`);
            const hoverEvent = new MouseEvent('mouseover', { view: window, bubbles: true, cancelable: true });
            item.dispatchEvent(hoverEvent); // 直接在列表项'a'上悬停
            await delay(500);

            console.log(`[步骤 2/4] 点击"选项"按钮以打开菜单...`);
            optionsButton.click(); // 直接用.click()更简洁
            await delay(500);

            console.log(`[步骤 3/4] 查找并点击菜单中的"删除"按钮...`);
            const deleteButton = document.querySelector('[data-testid="delete-chat-menu-item"]');
            if (deleteButton) {
                deleteButton.click();
                await delay(500);

                console.log(`[步骤 4/4] 查找并点击最终的"删除"确认按钮...`);
