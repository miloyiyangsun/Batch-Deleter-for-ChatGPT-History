...
async function hoverAndClickButtonsSequentially() {
    ...
    for (let i = 0; i < numberOfButtons; i++) {
        ...
        if (deleteButton) {
            ...
            console.log(`[步骤 4/5] 正在对"删除"按钮执行 pointerdown (按下) 操作...`);
            ...
            console.log(`[步骤 5/5] 正在对"删除"按钮执行 pointerup (抬起) 操作以完成点击...`);
            const deletePointerUpEvent = new PointerEvent('pointerup', {
                bubbles: true,
                cancelable: true,
                view: window
            });
            deleteButton.dispatchEvent(deletePointerUpEvent);
        } else {
