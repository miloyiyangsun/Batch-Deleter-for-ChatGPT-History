...
(async () => {
    ...
    const TIME_TO_WAIT_FOR_UI_TRANSITION = 500; // 用于等待菜单、弹窗等UI元素出现的延时
    const TIME_TO_WAIT_FOR_PAGE_REFRESH = 1500; // 用于等待删除操作后页面刷新的延时
    // -------------

    // 从 chrome.storage 中获取用户设定的删除数量
    const { deleteCount } = await chrome.storage.local.get('deleteCount');
    const NUMBER_OF_ITEMS_TO_DELETE = deleteCount || 3; // 如果未设定，默认值为 3
    ...
    /**
     * 最终版的自动化删除函数。
     * V4: 先记录前N个聊天标题作为锚点，然后根据锚点逐个定位并执行完整的删除流程。
     */
    async function deleteByTextAnchors() {
        ...
        // 提取前 N 个有效标题作为我们的目标锚点 (N 来自用户的输入)
